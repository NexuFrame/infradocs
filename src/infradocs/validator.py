...
class DataValidator:
    ...
    def _add_error(
        self, data_type: str, item_id: str, field: str, message: str
    ) -> None:
        """Add an error."""
        self.errors.append(
            {
        ...
        for item in items:
            ...
            for field in required:
                if field not in item or item[field] is None or item[field] == "":
                    self._add_error(
                        data_type, item_id, field, f"Missing required field: {field}"
                    )
    ...
    def _validate_field_type(
        self, data_type: str, item_id: str, field: str, value: Any
    ) -> None:
        ...
        if field_type == "ip":
            if not validate_ip_address(str(value)):
                self._add_error(
                    data_type, item_id, field, f"Invalid IP address: {value}"
                )
            ...
            if not validate_cidr(str(value)):
                self._add_error(
                    data_type, item_id, field, f"Invalid CIDR notation: {value}"
                )

        elif field_type == "mac":
            if not validate_mac_address(str(value)):
                self._add_error(
                    data_type, item_id, field, f"Invalid MAC address: {value}"
                )
            ...
            if not validate_datetime(str(value)):
                self._add_error(
                    data_type, item_id, field, f"Invalid datetime format: {value}"
                )
            ...
            try:
                ...
                self._add_error(
                    data_type, item_id, field, f"VLAN ID must be a number: {value}"
                )
            ...
            try:
                ...
                self._add_error(
                    data_type, item_id, field, f"Port must be a number: {value}"
                )
...
def validate_data(
    data: Dict[str, Any], require_private_ips: bool = True
...
def format_validation_errors(errors: List[Dict], warnings: List[Dict]) -> str:
    ...
    if errors:
        ...
        for err in errors:
            output.append(
                f"  [{err['type']}] {err['id']}.{err['field']}: {err['message']}"
            )
    ...
    if warnings:
        ...
        for warn in warnings:
            output.append(
                f"  [{warn['type']}] {warn['id']}.{warn['field']}: {warn['message']}"
            )
