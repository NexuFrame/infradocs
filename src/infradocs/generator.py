...
class SiteGenerator:
    ...
    def generate(
        self, data: Dict[str, Any], config: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        ...
        if data.get("sites"):
            ...
            stats["sites"] = (
                len(data["sites"]) if isinstance(data["sites"], list) else 1
            )
        ...
        if data.get("devices"):
            ...
            stats["devices"] = (
                len(data["devices"]) if isinstance(data["devices"], list) else 1
            )
        ...
        if data.get("vlans"):
            ...
            stats["vlans"] = (
                len(data["vlans"]) if isinstance(data["vlans"], list) else 1
            )
        ...
        if data.get("subnets"):
            ...
            stats["subnets"] = (
                len(data["subnets"]) if isinstance(data["subnets"], list) else 1
            )
        ...
        if data.get("printers"):
            ...
            stats["printers"] = (
                len(data["printers"]) if isinstance(data["printers"], list) else 1
            )
        ...
        if data.get("vendors"):
            ...
            stats["vendors"] = (
                len(data["vendors"]) if isinstance(data["vendors"], list) else 1
            )
        ...
        if data.get("circuits"):
            ...
            stats["circuits"] = (
                len(data["circuits"]) if isinstance(data["circuits"], list) else 1
            )
        ...
        if data.get("change_log"):
            ...
            stats["change_log"] = (
                len(data["change_log"]) if isinstance(data["change_log"], list) else 1
            )
            stats["files_generated"].append("change-log.html")

        if data.get("emergency_procedures"):
            self._generate_emergency_page(context)
            stats["emergency_procedures"] = (
                len(data["emergency_procedures"])
                if isinstance(data["emergency_procedures"], list)
                else 1
            )
    ...
    def _build_context(
        ...
        summary = {
            "total_sites": (
                ...
                else (1 if data.get("sites") else 0)
            ),
            "total_devices": (
                len(data.get("devices", []))
                if isinstance(data.get("devices"), list)
                else (1 if data.get("devices") else 0)
            ),
            "total_vlans": (
                len(data.get("vlans", []))
                if isinstance(data.get("vlans"), list)
                else (1 if data.get("vlans") else 0)
            ),
            "total_subnets": (
                len(data.get("subnets", []))
                if isinstance(data.get("subnets"), list)
                else (1 if data.get("subnets") else 0)
            ),
            "total_printers": (
                len(data.get("printers", []))
                if isinstance(data.get("printers"), list)
                else (1 if data.get("printers") else 0)
            ),
            "total_vendors": (
                len(data.get("vendors", []))
                if isinstance(data.get("vendors"), list)
                else (1 if data.get("vendors") else 0)
            ),
            "total_circuits": (
                len(data.get("circuits", []))
                if isinstance(data.get("circuits"), list)
                else (1 if data.get("circuits") else 0)
            ),
        }
