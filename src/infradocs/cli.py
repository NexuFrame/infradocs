...
def version() -> None:
    ...
    console.print("Generate static IT infrastructure documentation from YAML/JSON/CSV files.")
...
def init(
    ...
    if init_dir.exists():
        console.print(f"[yellow]Warning:[/yellow] Directory '{init_dir}' already exists.")
...
See the full documentation at: https://github.com/infradocs/infradocs-generator
"""
    ...
    console.print(f"  2. Run: [cyan]infradocs build --input {init_dir} --output {init_dir}/output[/cyan]")
    console.print(f"  3. Open [cyan]{init_dir}/output/index.html[/cyan] in your browser")
...
def build(
    ...
    try:
        stats = generate_site(data, str(templates_dir), str(output_path), config.to_dict())
...
def _show_generation_summary(stats: dict, output_path: Path) -> None:
    ...
    console.print(f"[bold]Output location:[/bold] [cyan]{output_path.absolute()}/index.html[/cyan]")
...
def _create_sample_data(init_dir: Path) -> None:
    ...
    # Sites
    _write_file(init_dir / 'sites.yaml', """# Physical sites/locations
- id: site-hq
  name: "Headquarters"
...
""")
    ...
    # Devices
    _write_file(init_dir / 'devices.yaml', """# Network devices
- id: fw-01
  name: "Main Firewall"
  type: "firewall"
...
""")
    ...
    # VLANs
    _write_file(init_dir / 'vlans.yaml', """# VLAN configurations
- id: vlan-mgmt
  name: "Management"
  vlan_id: 10
...
""")
    ...
    # Subnets
    _write_file(init_dir / 'subnets.yaml', """# IP subnets
- id: subnet-mgmt
...
""")
    ...
    # Printers
    _write_file(init_dir / 'printers.csv', """id,name,ip_address,location,type,status
printer-01,Office Printer,192.168.1.50,Headquarters - Floor 2,Laser,online
""")
    
    # Vendors
    _write_file(init_dir / 'vendors.yaml', """# Vendors and suppliers
- id: vendor-cisco
  name: "Cisco Systems"
...
""")
    ...
    # Circuits
    _write_file(init_dir / 'circuits.yaml', """# Internet/telecom circuits
- id: circuit-internet
...
""")
    ...
    # Change log
    _write_file(init_dir / 'change-log.yaml', """# Change log entries
- id: change-001
  date: "2024-01-15"
...
""")
    ...
    _write_file(init_dir / 'emergency-procedures.yaml', """# Emergency procedures
